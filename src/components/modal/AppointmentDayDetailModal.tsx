...
function Body({ isOpen, appointmentId, onClose, onCancel, showCancel = true }: Props) {
    const { data: appt, isLoading } = useQuery({
        queryKey: ["appointmentDetail", appointmentId],
        queryFn: (): Promise<AppointmentDetailFromApi> =>
            getAppointmentDetailById(appointmentId!),
        enabled: !!appointmentId,
        select: (dataFromApi) => {
            if (!dataFromApi) return undefined;
            const participantsString =
                dataFromApi.participants && Array.isArray(dataFromApi.participants)
    ...
    const formatDateTimeRange = (start?: string, end?: string) => {
        ...
        if (startDate.isSame(endDate, "day")) {
            ...
            return `${startDate.format("YYYY년 MM월 DD일")} ~ ${endDate.format(
                "YYYY년 MM월 DD일"
            )}`;
        }
    };
            ...
            {!isLoading && appt && (
                <div className="relative w-full max-w-[810px]">
                    <div
                        ...
                        style={{
                            backgroundColor: COLOR_TOKEN[appt.color] || "#E2F5E4",
                        }}
                    >
                        <div className="bg-white shadow-2xl max-h-[85vh] overflow-hidden flex flex-col">
                            ...
                            <div className="px-6 pb-6 font-pretendard">
                                <div className="bg-[#EBEDF0] p-5 md:p-6">
                                    <p className="font-kccganpan text-[20px] mb-3 ">
                                        To. {appt.to}
                                    </p>
                                    ...
                                    <p className="mt-3 text-right font-kccganpan text-[20px]">
                                        From. {appt.proposeUserName}
                                    </p>
                                </div>
                            </div>
                        </div>
