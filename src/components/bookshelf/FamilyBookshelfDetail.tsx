...
const useCreateCustomQuestion = () => {
    ...
    return useMutation({
        ...
        onError: (error: AxiosError) => {
            ...
            if (code === "GEN4091") {
                FailToast("이미 존재하는 질문입니다.");
            } else {
                FailToast("질문 생성에 실패했습니다. 잠시 후 다시 시도해 주세요.");
            }
            console.error("질문 생성 중 에러 발생:", error.message);
        },
    });
};
...
export default function FamilyBookshelfDetailPage() {
    ...
    const doServerSave = useCallback(async () => {
        if (!isEditable || !isDirty || isSaving) {
            return;
        }
        try {
            setIsSaving(true);
    ...
    }, [entries, isEditable, isDirty, isSaving]);

    useEffect(() => {
        if (!isEditable) return;
        const iv = setInterval(() => {
            void doServerSave();
        }, 3000); // 3초 간격으로 자동 저장 실행 
        return () => clearInterval(iv);
    }, [isEditable, doServerSave]);
                    ...
                    <div className="px-6 pt-6 pb-3">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                ...
                                {isEditable && (
                                    <span
                                        // [수정 7] ref.current 대신 state 변수를 사용하여 클래스와 텍스트를 결정합니다.
                                        className={`ml-3 text-xs font-pretendard ${isSaving ? "text-[#4C505C]" : saveError ? "text-red-500" : "text-[#7A7A7A]"
                                            }`}
                                    >
                                        {isSaving
                                            ? "자동 저장 중..."
                        ...
                        {entries.map((it, idx) => (
                            <div key={it.id} className="mb-5 rounded-2xl bg-white p-5">
                                <p className="font-kccganpan text-[22px] text-[#49684A] mb-3 flex justify-between items-center">
                                    <span>{idx > 14 ? "+" : idx + 1}. {it.question}</span>
                                    {isEditable && it.id > 15 && (
                                        <button
